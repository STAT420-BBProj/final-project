...
plot_fitted_versus_residuals <- function(fitted_values, residuals, model_name) {
  plot(fitted(bic_model), resid(bic_model), 
       col = "grey", 
       pch = 20, 
       xlab = "Fitted", 
       ylab = "Residuals", 
...
model_diagnostics = function(model) {
  ...
  cd_found <- cooks.distance(model) > 4 / n
  print(paste("Number of Influential Observations: ", sum(cd_found)))
}
```
...
```{r}
####  Step Search - AIC Model: Unusual Observtions -- Cooks Distance > 4 / n
cd_found <- cooks.distance(bic_forward_model) > 4 / n
print(paste("Number of Influential Observations: ", sum(cd_found)))
```

```{r}
bbproj_trn[which(cd_found),]
```
...
```{r}
####  Step Search - AIC Model: Unusual Observtions -- Cooks Distance > 4 / n
cd_found <- cooks.distance(aic_forward_model) > 4 / n
print(paste("Number of Influential Observations: ", sum(cd_found)))
```
