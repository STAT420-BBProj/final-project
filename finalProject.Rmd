...
On base percentage is essentially the ratio of the number of times a batter reaches base divided by the number plate appearances.  OBP is a measure of the teams batting efficiency.

### OPS

`OPS`, or **o**n-base percentage **p**lus **s**lugging percentage, is calculated exactly how it's described.
...
`WHIP`, or walks and hits per innings pitched, is a derived pitching statistic used to evaluate a pitcher's ability to throw "clean" innings.
...
Batting average on balls-in-play is a measure of a hitter's ability to get a hit while keeping the ball in play (i.e. *without* home runs).
...
Runs Created, or `RC`, is an attempt to directly measure the number of runs a player creates for his team.
...
`wOBA`, or weighted on-base average, is similar to on-base percentage `OBP`, giving "extra credit" for extra-base hits, but not as much credit as slugging percentage `SLG`.
