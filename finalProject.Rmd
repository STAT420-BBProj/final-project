...
Graphically, there seems to be a moderate influence on a team's salary and their chances of taking home a division crown.  Indeed, our Wald test for `salary` alone yields a p-value of `r salary_only_summary$coefficients["salary", "Pr(>|z|)"]`, allowing us to reject the null hypothesis
...
We will begin our search for a better classifier by setting up an initial logisitc regression model contain all of the predictors we would like to evaluate.  Then, we will proceed to eliminate predictors using backwards, forwards, and stepwise AIC and BIC searches.
...
Our backwards search using BIC yields the following logistic model

\[
\log \bigg(\frac{P[DivWin = 1]}{1 - P[DivWin = 1]} \bigg) = \beta_0 + \beta_W W + \beta_{AB} AB + \beta_H H + \beta_{HR} HR + \beta_{BB} BB + \beta_{HBP} HBP + \beta_{IBB} IBB + \beta_{SLG} SLG + \beta_{OBP} OBP + \beta_{wOBA} wOBA
\]

where the log odds of a team winning their division is dependent upon wins, at bats, hits, home runs, walks, hits by pitch, intentional walks, slugging percentage, on-base percentage, and weighted on-base average.

#### Diagnostics: BIC Model

Next, we will evaluate this model using a confusion matrix and calculating it sensitivity, specificity, and its misclassification rate.

```{r}
bic_model <- glm(DivWin ~ W + AB + H + HR + BB + HBP + IBB + SLG + OBP + wOBA, family = binomial, data = bbproj_trn)
...
Here, we beat the salary only model, and our prevalence, with a misclassification rate of `r round(bic_misclass, 3)`.  Our specificity looks great for our backward BIC model at `r round(bic_specificity, 3)`.  Our `r bic_conf_matrix[1, 2]` false negatives could come down, however, with an adjustment to our cutoff value.

#### Reset Cutoff to 0.25

Let's start with a cutoff of 0.25:

```{r}
bic_prediction_25 <- ifelse(predict(bic_model, bbproj_tst, type = "response") > 0.25, "Y", "N")

(bic_misclass_25 = mean(bic_prediction_25 != bbproj_tst$DivWin))

(bic_conf_matrix_25 = table(prediction = bic_prediction_25, actual = bbproj_tst$DivWin))

(bic_sensitivity_25 = bic_conf_matrix_25[2, 2] / sum(bic_conf_matrix_25[, 2]))

(bic_specificity_25 = bic_conf_matrix_25[1, 1] / sum(bic_conf_matrix_25[, 1]))
```

Interestingly, our misclassification rate went down with our cutoff adjustment to `r round(bic_misclass_25, 3)`.  Additionally, our sensitivity went up significantly (`r round (bic_sensitivity_25, 3)`) and we only had a modest reduction in specificity (`r round(bic_specificity_25, 3)`).  This model looks to be exceptionally adept at predicting division winners, and `salary` is not even a predictor!
